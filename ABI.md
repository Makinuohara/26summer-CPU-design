...
> FPGA 顶层按 `{BTNC, BTNU, BTNR, BTND, BTNL}` 连接为 `btn[4:0]`。地址 `0x8000_0030` 保留给数码管 `DIGIT7`，因此按钮位于下一字地址 `0x8000_0034`。按下产生上升沿中断，读取本寄存器会清除 pending；松开本身不触发中断。写操作触发总线错误。
...
| 3 | **无软件中断** | msip 硬连线为 0 |
| 4 | **按键仅按下触发** | 只在同步后的上升沿置 pending，松开不产生新的中断 |
