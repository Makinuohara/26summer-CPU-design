...
为满足拓展层次中“浮点或乘除法扩展”的要求，当前版本选择实现整数乘除法核心子集，不实现浮点运算。

| 指令                 | 类型   | 编码关键字段                                           | 语义                               | 当前实现 |
| -------------------- | ------ | ------------------------------------------------------ | ---------------------------------- | -------- |
| `mul rd, rs1, rs2` | R-type | `opcode=0110011`，`funct7=0000001`，`funct3=000` | `rd = low32(rs1 * rs2)`          | 已支持   |
| `div rd, rs1, rs2` | R-type | `opcode=0110011`，`funct7=0000001`，`funct3=100` | `rd = signed(rs1) / signed(rs2)` | 已支持   |

当前 `mul/div` 复用 EX 阶段 ALU 通路，由硬布线译码器直接根据 `opcode/funct3/funct7` 生成新的 `alu_ctrl`：

```text
instr[6:0]   = 0110011  -> R-type ALU
instr[31:25] = 0000001  -> M 扩展乘除法分支
instr[14:12] = 000      -> MUL
instr[14:12] = 100      -> DIV
```
...
1. `mul` 返回乘积低 32 位，不实现 `mulh/mulhsu/mulhu`。
2. `div` 为有符号除法；除数为 0 时返回 `0xffff_ffff`，`0x8000_0000 / -1` 返回 `0x8000_0000`，与 RV32M 语义保持一致。
3. 当前实现为组合 ALU 版本，便于快速上板展示；若后续频率不足，可替换为多周期乘除法单元，并通过流水线 stall/ready 握手接入。
...
```text
addr < 0x0800_0000                         -> 主存访问，走 CPU 内部 D-Cache
0x8000_0000 - 0x8000_0007                  -> PS/2 MMIO，旁路 D-Cache
0x8000_0008 - 0x8000_000B                  -> SW MMIO，旁路 D-Cache
0x8000_000C - 0x8000_000F                  -> LED MMIO，旁路 D-Cache
0x8000_0010 - 0x8000_002F                  -> SEG MMIO，旁路 D-Cache
0x8000_0030 - 0x8000_0033                  -> BTN MMIO，旁路 D-Cache
0x8100_0000 - 0x8120_0007                  -> interrupt_controller MMIO，旁路 D-Cache
其他地址                                      -> CPU 近端直接返回 fault
```
...
3. 非法地址路径：`MEM stage -> CPU near decoder -> fault`

这样做的好处是：

- cache 不再和后端主存封装在同一个模块中
- 主存层次、I/O 层次和非法地址在 CPU 侧就已分流
- 更符合“cache 作为 CPU 近端缓存”的设计本意
- cache 只看真实主存地址，不会因为设备映射地址污染 cache line
...
- 仅缓存主存地址空间
- 合法 MMIO 永远不进入 cache，而是直通 SoC 侧 `dmem_bus_decoder`
- 非法地址不会发到外部总线，CPU 直接产生访存 fault 响应
